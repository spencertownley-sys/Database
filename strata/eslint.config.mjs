    ...
    rules: {
      ...
      'no-restricted-syntax': [
        'error',
        {
          ...
          selector:
            "TaggedTemplateExpression[tag.name='sql'] > TemplateLiteral BinaryExpression[operator='+']",
          message:
            'No string concatenation inside a `sql` tagged template — interpolate a bound parameter instead (see filterCompiler.ts).',
        },
        {
          selector:
            "TaggedTemplateExpression[tag.name='sql'] > TemplateLiteral CallExpression[callee.property.name='join']",
          message:
            'No Array.join() inside a `sql` tagged template — use sql.join()/sql.raw with a vetted allowlist instead.',
        },
        {
