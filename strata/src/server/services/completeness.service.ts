/**
 * Completeness: "how much of this item's required data is actually filled in".
 *
 * PRD §4.7 defines the metric exactly: **filled required fields ÷ total
 * required fields**. Optional fields do not appear in the denominator, and
 * neither does the title — completeness answers "is the data someone declared
 * mandatory present", not "how many cells are non-empty". A broader
 * denominator makes every item read as less complete than the workspace's own
 * rules say it is, and this number is filterable, sortable, aggregated per
 * group, and the basis of the "incomplete only" toggle.
 *
...
export interface CompletenessInput {
  ...
  fields: readonly Pick<Field, 'key' | 'type' | 'requiredForCompleteness' | 'config'>[];
...
export interface CompletenessResult {
  /** 0–100, integer. */
  pct: number;
  /** Keys of required fields that are empty, in field order. Powers the detail panel. */
...
export function computeCompleteness(input: CompletenessInput): CompletenessResult {
  const { effectiveValues, fields, invalidValues = {} } = input;
  ...
  for (const field of fields) {
    if (!field.requiredForCompleteness) continue;

    required += 1;
    const value = effectiveValues[field.key];
    const isInvalid = Object.prototype.hasOwnProperty.call(invalidValues, field.key);
    if (isInvalid || handlerFor(field.type).isEmpty(value)) missingRequired.push(field.key);
    else filled += 1;
  }

  // An Item Type with no required fields is complete, not divide-by-zero.
  const pct = required === 0 ? 100 : Math.round((filled / required) * 100);

  return { pct, missingRequired, requiredFields: required, filledFields: filled };
}
